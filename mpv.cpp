...
namespace ImPlay {
    ...
    void Mpv::waitEvent(double timeout) {
        while (mpv) {
            ...
            case MPV_EVENT_PROPERTY_CHANGE: {
                ...
                //if (prop->name == "sub-text")
                //{
                //    std::cout << "d";
                //    char timepos = *(char*)prop->data;
                ...
                if (strcmp(prop->name, "sub-text") == 0)
                {
                    ...
                    //lem = "jaos";
                    try {
                        if (prop->data != NULL && prop->data != nullptr)
                        {
                            char* value = *(char**)(prop->data);

                            std::string subtitle = *(const char**)prop->data;
    ...
    void Mpv::init(GLAddrLoadFunc load, int64_t wid) {
        ...
        //CRAVEN
        const char* cmd[] = { "loadfile", "C:\\Users\\Brandon\\Videos\\33.mp4", NULL };
        mpv_command(mpv, cmd);
    ...
    void Mpv::observeProperties() {
        ...
        //mpv_observe_property(ctx, 0, "sub-text", MPV_FORMAT_STRING);
        observeProperty<char*, MPV_FORMAT_STRING>("sub-text", [this](char* data) { subtext = data; });
